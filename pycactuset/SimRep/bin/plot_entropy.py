...
def plot_entr(entr_max, entr_avg, uentr_avg, thform):
  ...
  if (entr_max is not None):
    ax.semilogy(entr_max.t / cu_ms, entr_max.y, 'k-', label=r'$\max(s)$')
  #
  if (entr_avg is not None):
    ax.semilogy(entr_avg.t / cu_ms, entr_avg.y, 'g-', label=r'$\bar{s}$')
  #
  if (uentr_avg is not None):
    ax.semilogy(uentr_avg.t / cu_ms, uentr_avg.y, 'r-', label=r'$\bar{s}$ unbound')
  #

  ax.legend(loc='best')
  #ax.set_ylim(ymin=0)#, ymax= entr_max.y.max())
