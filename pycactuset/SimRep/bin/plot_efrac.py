...
def plot_efrac(minefrac, maxefrac, avgefrac):
  ...
  if minefrac:
    ax.plot(minefrac.t / cu_ms, minefrac.y, 'b-', label=r'$\min(Y_e)$')
  if maxefrac:
    ax.plot(maxefrac.t / cu_ms, maxefrac.y, 'r-', label=r'$\max(Y_e)$')
  if avgefrac:
    ax.plot(avgefrac.t / cu_ms, avgefrac.y, 'g-', label=r'$\bar{Y}_e$ unbound')

  ax.legend(loc='best')
  ax.set_xlim(xmin=minefrac.t.min() / cu_ms, xmax= minefrac.t.max() / cu_ms)
...
def main(opt, args):
  ...
  uye_tot    = sd.ts.absint.get('ye_unbnd')
