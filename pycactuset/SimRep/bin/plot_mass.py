...
def plot_mass(mb_tot, mb_noatmo, m_adm_vol, m_adm_qlm, tah, mah, mir):
  ...
  ax.plot(mb_tot.t / cu_ms, mb_tot.y, 'k-', label=r'$M_b$ (total)')
  if mb_noatmo is not None:
    ax.plot(mb_noatmo.t / cu_ms, mb_noatmo.y, 'b-', label=r'$M_b$ (no atmo)') 
    y     = mb_tot.y - mb_noatmo.y
    ax.plot(mb_tot.t / cu_ms, y, 'r-',
            label=r'$M_b$ (artificial atmo)')
  #
  if m_adm_vol is not None:
    ax.plot(m_adm_vol.t / cu_ms, m_adm_vol.y, ls='-',color='darkgreen', 
            label=r'$M_\mathrm{ADM}$ (vol.)')
  #
  clrs = ['Olive', 'cyan']
  for rsf, m_adm_sf in m_adm_qlm[-2:]:
    lbl = r'$M_\mathrm{ADM}$ ($r=%.1f$ km)' % (rsf / cu_km)
    ax.plot(m_adm_sf.t / cu_ms, m_adm_sf.y, '-', color=clrs.pop(0), label=lbl)
  #
  if mah is not None:
    ax.plot(mah.t / cu_ms, mah.y, ls='--',color='green', 
            label=r'$M_\mathrm{BH}$')
  #
  if mir is not None:
    ax.plot(mir.t / cu_ms, mir.y, ls=':',color='green', 
...
def plot_cons(mb_t, mb_na, m_adm_vol, m_adm_qlm, mb_oh, mb_na_oh, tah):
  ...
  y     = relc(mb_t)
  ax.plot(mb_t.t / cu_ms, y, ls='-', color='k', label=r'$M_b$ (total)')
  ...
  if mb_oh is not None:
    ...
    ax.plot(mb_oh.t / cu_ms, y, ls=':', color='k', label=r'$M_b$ (outside AH)')
  #
  if mb_na is not None:
    y     = relc(mb_na)
    ax.plot(mb_na.t / cu_ms, y, ls='-', color='b', label=r'$M_b$ (no atmo)')
    y     = 1-mb_na.y/mb_t.y
    #ymax  = max(ymax, max(y))
    ax.plot(mb_t.t / cu_ms, y, 'r-',
            label=r'$M_b$ (artificial atmo)')
  #
  if mb_na_oh is not None:
    ax.plot(mb_na_oh.t / cu_ms, mb_na_oh.y/mb_t.y[0], ':', color='b',
  ...
  if m_adm_vol is not None:
    ...
    ax.plot(m_adm_vol.t / cu_ms, y, '-', color='darkgreen', 
  ...
  for rsf, m_adm_sf in m_adm_qlm[-2:]:
    ...
    ymin = min(ymin, min(y))
    ymax = max(ymax, max(y))
    lbl = r'$M_\mathrm{ADM}$ (surface $r=%.1f$ km)' % (rsf / cu_km)
    ax.plot(m_adm_sf.t / cu_ms, y, ls='-', color=clrs.pop(0), label=lbl)
  #
  if tah is not None:
    ax.axvline(x=tah / cu_ms, ls='-', lw=1, color='darkslategrey')
  #  
