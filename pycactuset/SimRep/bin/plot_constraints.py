...
def plot_ham(mham, mmom, rho0, thform, tmax):
  ...
  if mham:
    ...
    ax.semilogy(mham.t / cu_ms, nham, '-', label=r'$L_\infty(H)$')
  #
  for d,m in enumerate(mmom):
    if m:
      ...
      ax.semilogy(m.t / cu_ms, nm, '-', label=lbl)
    #
  #
  if (thform != None):
  ...
  ax.legend(loc='best')
  #ax.set_ylim(ymin=0)
  t1 = tmax if tmax!=None else t1 / cu_ms
...
def plot_ham_norm1(mham, mmom, rho0, thform, tmax):
  ...
  if mham:
    t1  = max(t1, mham.t[-1])
    nham  = mham.y / (16*pi*rho0)
    ax.semilogy(mham.t / cu_ms, nham, '-', label=r'$L_1(H)$')
  #
  for d,m in enumerate(mmom):
    if m:
      t1  = max(t1, m.t[-1])
...
def plot_ham_norm2(mham, mmom, rho0, thform, tmax):
  ...
  if mham:
    t1  = max(t1, mham.t[-1])
    nham  = mham.y / (16*pi*rho0)
    ax.semilogy(mham.t / cu_ms, nham, '-', label=r'$L_2(H)$')
  #
  for d,m in enumerate(mmom):
    if m:
      t1  = max(t1, m.t[-1])
